...
def clean_node_name(node_name):
    '''cleans the node name to remove problematic characters and avoid reserved word conflicts'''
...
def eval_children(in_dict):
    ...
    for node_name in in_dict:
        ...
        if this_node['parents'][0] != None: # if this node is not the root and no other node can have a None parent, so we don't need to collect parents 
            for parent in this_node['parents']:
                if node_name not in in_dict[parent]['children']:
                    print 'eval_children: adding child reference to parent', node_name, parent
...
def file_header(name):
    ...
    tmp_block.append('')
    tmp_block.append('rewrite_name_list = ("name", "value", "attrs", "flatten_self", "flatten_self_safe_sql_attrs", "flatten_self_to_utf8", "children")')
    tmp_block.append('')
    ...
    tmp_block.append('')
    tmp_block.append('def clean_node_name(node_name):')
    tmp_block.append('    clean_name = node_name.replace(":", "_").replace("-", "_").replace(".", "_")')
    tmp_block.append('')
    tmp_block.append('    if clean_name in rewrite_name_list:')
    tmp_block.append('        clean_name = "_" + clean_name + "_"')
    tmp_block.append('')
    tmp_block.append('    return clean_name')
    tmp_block.append('')
    return '\n'.join(tmp_block)+'\n'

######### START HERE AND SWAP IN CLEAN_NAME FOR EXPLICIT NAME CLEANUPS...
...
def create_handler(in_dict):
    ...
    tmp_block.append('\t\tself.last_processed = None')
    ...
    tmp_block.append("\t\tname = clean_node_name(name)")
    ...
    tmp_block.append('\t\tself.char_buffer = []')
    tmp_block.append('\t\tself.last_processed = "start"')
    ...
    tmp_block.append("\t\tname = clean_node_name(name)")
    tmp_block.append('')
    tmp_block.append('\t\tif (len(self.char_buffer) != 0) and (self.last_processed == "start"):')
    ...
    for node_name in in_dict:
        ...
        if this_node['parents'][0] == None:
            ...
            tmp_block.append('\t\t\tself.char_buffer = []')        
            tmp_block.append('\t\t\tself.last_processed = "end"')
            tmp_block.append('\t\t\treturn')
        else:
            tmp_block.append('\t\telif name == "'+node_name+'":')   # grab each node
...
class NodeHandler(ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        collect_node(self.nodes, {'name': clean_node_name(name),
    ...
    def endElement(self, name):
        ...
        if self.depth[-1] == clean_node_name(name):
            self.depth.pop()
        else: print "ERROR: unexpected node", clean_node_name(name), "found instead of", self.depth[-1]
